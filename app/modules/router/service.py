...
class RouterService:
    ...
    def _execute_via_vps(self, router: Router, command: str, **kwargs) -> List[Dict[str, Any]]:
        ...
        script = '\n'.join([
            ...
            "  ss('/login','=name='+username,'=response='+rh)",
            "  replies=rs()",
            "for r in replies:",
    ...
    def _execute_client_method_via_vps(self, router: Router, method_name: str, **kwargs) -> Any:
        if method_name == 'get_router_info':
            ...
            return {'hostname': i0.get('name'), 'version': r0.get('version'), 'uptime': r0.get('uptime'),
                    ...
                    'architecture_name': r0.get('architecture-name'), 'platform': r0.get('platform')}
        ...
        elif method_name == 'get_hotspot_users': return self._execute_via_vps(router, '/ip/hotspot/user/print', **kwargs.get('params', {}))
        elif method_name == 'get_pppoe_secrets': return self._execute_via_vps(router, '/ppp/secret/print')
        elif method_name == 'get_active_sessions': return self._execute_via_vps(router, '/ip/hotspot/active/print', **kwargs.get('params', {}))
        elif method_name == 'get_pppoe_active_sessions': return self._execute_via_vps(router, '/ppp/active/print')
        elif method_name == 'get_hotspot_profiles': return self._execute_via_vps(router, '/ip/hotspot/user/profile/print')
        elif method_name == 'get_interface_stats': return self._execute_via_vps(router, '/interface/print')
        elif method_name == 'get_simple_queues': return self._execute_via_vps(router, '/queue/simple/print')
            ...
            try: self._execute_via_vps(router, '/ip/firewall/filter/add', chain='input', src_address='10.0.0.0/16', action='accept', comment='Allow ISP Platform', **{'place-before': '0'})
            except: pass
            existing = self._execute_via_vps(router, '/radius/print'); found = False
            for item in existing:
                if item.get('address') == rs:
                    self._execute_via_vps(router, '/radius/set', numbers=item.get('.id'), secret=secret, service='hotspot,ppp', authentication_port='1812', accounting_port='1813', timeout='1s')
                    found = True; break
            if not found: self._execute_via_vps(router, '/radius/add', address=rs, secret=secret, service='hotspot,ppp', authentication_port='1812', accounting_port='1813', timeout='1s')
    ...
    # =========================================================================
    # COMPLETE SETUP SCRIPT — EVERY COMMAND
    # =========================================================================
...
# STEP 2: Firewall — Allow ISP Platform (place-before=0 puts it FIRST)
...
# STEP 3: Enable API, Winbox, and SSH for platform management
...
# STEP 5: Enable RADIUS on Hotspot Profiles
/ip hotspot profile set [find] use-radius=yes

# STEP 6: Enable RADIUS on PPPoE
/ppp aaa set use-radius=yes

# STEP 7: Enable RADIUS Incoming (for remote disconnects)
/radius incoming set accept=yes

# STEP 8: Walled Garden — Allow captive portal access before login
...
"""
    ...
    def auto_configure_after_wireguard(self, router_id: UUID, organization_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            result['radius_configured'] = True; result['steps'].append({'step': 'radius', 'status': 'success'})
        except Exception as e:
        ...
        try:
            info = self._execute_client_method_via_vps(router, 'get_router_info'); caps = ['api']
            try:
                if self._execute_client_method_via_vps(router, 'get_hotspot_servers'): caps.append('hotspot')
            except: pass
            try:
                if self._execute_client_method_via_vps(router, 'get_pppoe_servers'): caps.append('pppoe')
            except: pass
            ...
            result['discovered'] = True; result['steps'].append({'step': 'discovery', 'status': 'success'})
        except Exception as e: result['steps'].append({'step': 'discovery', 'status': 'error', 'error': str(e)})
