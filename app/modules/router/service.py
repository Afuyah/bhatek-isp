...
class RouterService:
    ...
    def _execute_via_vps(self, router: Router, command: str, **kwargs) -> List[Dict[str, Any]]:
        ...
        script = '\n'.join([
            ...
            " if l<0x4000:l|=0x8000;return bytes([(l>>8)&0xFF,l&0xFF])",
            " if l<0x200000:l|=0xC00000;return bytes([(l>>16)&0xFF,(l>>8)&0xFF,l&0xFF])",
            " if l<0x10000000:l|=0xE0000000;return bytes([(l>>24)&0xFF,(l>>16)&0xFF,(l>>8)&0xFF,l&0xFF])",
            " return bytes([0xF0,(l>>24)&0xFF,(l>>16)&0xFF,(l>>8)&0xFF,l&0xFF])",
            "def ew(w):",
            " wb=w.encode()",
            " return el(len(wb))+wb",
            "",
            "# Read variable-length integer from socket",
            "def rl(s):",
            " b=s.recv(1)",
            " if not b:return -1",
            " b=b[0]",
            ...
            " if b&0xC0==0x80:",
            "  r=s.recv(1)",
            "  if r:return((b&0x3F)<<8)|r[0]",
            "  return -1",
            " if b&0xE0==0xC0:",
            "  r=s.recv(2)",
            "  if len(r)==2:return((b&0x1F)<<16)|(r[0]<<8)|r[1]",
            "  return -1",
            " if b&0xF0==0xE0:",
            "  r=s.recv(3)",
            "  if len(r)==3:return((b&0x0F)<<24)|(r[0]<<16)|(r[1]<<8)|r[2]",
            "  return -1",
            " if b&0xF8==0xF0:",
            "  r=s.recv(4)",
            "  if len(r)==4:return((b&0x07)<<32)|(r[0]<<24)|(r[1]<<16)|(r[2]<<8)|r[3]",
            "  return -1",
            " return -1",
            "",
            ...
            " sock.sendall(el(0))",
            "",
            "# Read a sentence (until !done/!trap/!fatal)",
            "def rs():",
            " replies=[]",
            " cur={}",
            " done=False",
            ...
            " return replies",
            "",
            "# AUTHENTICATION — Modern login with legacy fallback",
            ...
            "  md.update(password)",
            "  md.update(cb)",
            "  ss('/login','=name='+username,'=response='+md.hexdigest().upper())",
            "  replies=rs()",
            "",
