...
class MikroTikConnection:
    ...
    def _login(self) -> None:
        ...
        3. Calculate MD5(challenge + password + challenge)
    ...
    def _compute_challenge_response(self, challenge: str) -> str:
        ...
        Per RouterOS protocol: prepend null byte, then password, then hex-decoded challenge.
    ...
    def _read_exact(self, size: int) -> bytes:
        ...
        while len(data) < size:
            try:
                ...
                if not chunk:
                    raise MikroTikConnectionError(
                        f"Connection closed by {self.host} (received {len(data)} of {size} bytes)"
                    )
                data += chunk
...
class MikroTikClient:
    ...
    # Maximum connections per unique router key before cleanup triggers
    MAX_CONNECTIONS_PER_ROUTER = 10
    ...
    def _cleanup_oldest_connections(self) -> None:
        ...
        # Sort by last_used (oldest first) and remove the 10 oldest
        sorted_conns = sorted(
    ...
    def invalidate_connection(self, router_data: Dict[str, Any]) -> None:
        ...
        Call this when you know the connection is bad (e.g., after repeated failures).
        """
        ...
        **kwargs
        ...
        for attempt in range(retries):
            try:
                ...
                last_error = e
                logger.warning(
                    f"Command '{command}' failed (attempt {attempt + 1}/{retries}) "
                ...
                # Non-connection errors (auth, command errors) should not retry
                raise
    ...
    def configure_radius(
        ...
        try:
            ...
            # Enable RADIUS for PPP (PPPoE)
            try:
                self.execute(router_data, '/ppp/set', **{'use-radius': 'yes'})
            except MikroTikAPIError:
                logger.debug(f"PPP RADIUS not configured (may not have PPPoE)")
                pass

            # Enable RADIUS incoming for CoA/Disconnect packets
            try:
                self.execute(router_data, '/radius/incoming/set', accept='yes')
            except MikroTikAPIError:
                logger.debug(f"RADIUS incoming not configurable on this router")
                pass

            logger.info(
                f"RADIUS fully configured on {router_data.get('ip_address')} "
            ...
            logger.error(
                f"Failed to configure RADIUS on {router_data.get('ip_address')}: {e}"
            )
    ...
    def create_hotspot_user(
        ...
        try:
            ...
            logger.info(
                f"Created hotspot user '{username}' on {router_data.get('ip_address')}"
            )
    ...
    def set_hotspot_user(
        ...
        **kwargs
        ...
        try:
            ...
            logger.info(f"Updated hotspot user '{username}' on {router_data.get('ip_address')}")
            return {'success': True, 'username': username}
        except Exception as e:
            logger.error(f"Failed to update hotspot user '{username}': {e}")
            return {'success': False, 'error': str(e)}
    ...
    def remove_hotspot_user(
        ...
        try:
            ...
            logger.info(f"Removed hotspot user '{username}' from {router_data.get('ip_address')}")
            return {'success': True, 'username': username}
        except Exception as e:
    ...
    def disconnect_hotspot_user(
        ...
        try:
            ...
            logger.info(f"Disconnected hotspot user '{username}' from {router_data.get('ip_address')}")
            return {'success': True, 'username': username}
        except Exception as e:
        ...
        try:
            ...
            for profile in result:
                profiles.append({
                    ...
                    'transparent_proxy': profile.get('transparent-proxy') == 'true',
    ...
    def create_hotspot_profile(
        ...
        try:
            ...
            logger.info(f"Created hotspot profile '{name}' on {router_data.get('ip_address')}")
            return {'success': True, 'name': name}
        except Exception as e:
            logger.error(f"Failed to create hotspot profile '{name}': {e}")
            return {'success': False, 'error': str(e)}
    # PPPoE MANAGEMENT
    ...
    def create_pppoe_secret(
        ...
        Note: For RADIUS-based authentication (the primary path), PPPoE secrets
        should be managed via RadCheck entries, not directly on the router.
        """
        params: Dict[str, Any] = {
            'name': username,
            'password': password,
        ...
        try:
            ...
            logger.info(
                f"Created PPPoE secret '{username}' on {router_data.get('ip_address')}"
            )
            return {'success': True, 'username': username}
        except Exception as e:
    ...
    def set_pppoe_secret(
        ...
        **kwargs
        ...
        try:
            ...
            logger.info(f"Updated PPPoE secret '{username}' on {router_data.get('ip_address')}")
            return {'success': True, 'username': username}
        except Exception as e:
            logger.error(f"Failed to update PPPoE secret '{username}': {e}")
            return {'success': False, 'error': str(e)}
    ...
    def remove_pppoe_secret(
        ...
        try:
            ...
            logger.info(f"Removed PPPoE secret '{username}' from {router_data.get('ip_address')}")
            return {'success': True, 'username': username}
        except Exception as e:
    ...
    def disconnect_pppoe_user(
        ...
        try:
            ...
            logger.info(f"Disconnected PPPoE user '{username}' from {router_data.get('ip_address')}")
            return {'success': True, 'username': username}
        except Exception as e:
    ...
    def set_bandwidth_limit(
        ...
        try:
            ...
            logger.error(f"Failed to set bandwidth limit for {target}: {e}")
            return {'success': False, 'error': str(e)}
    ...
    def remove_simple_queue(
        ...
        try:
            ...
            logger.info(f"Removed simple queue '{queue_identifier}' from {router_data.get('ip_address')}")
            return {'success': True, 'queue': queue_identifier}
        except Exception as e:
            logger.error(f"Failed to remove simple queue '{queue_identifier}': {e}")
            return {'success': False, 'error': str(e)}
    # CONNECTION LIFECYCLE
